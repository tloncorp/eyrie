...
4. From the `ui` directory you can run `rsync -avL --delete dist/ ~/zod/work/eyrie` where `~/zod` is your fake urbit's pier.
...
7. You can now run `-make-glob %work /eyrie` which will take the folder where you just added files and create a glob which can be thought of as a sort of bundle. It will be output to `~/zod/.urb/put`.
