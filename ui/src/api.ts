...
export class Urbit {
  ...
  /**
   * Controls whether connection gets killed when tab is hidden
   */
  private openWhenHidden: boolean = true;
  ...
  constructor(public url: string, public code?: string, public desk?: string) {
    ...
    useEyreState.getState().reset(this.uid);
    return this;
  }
  ...
  async connect(): Promise<void> {
    ...
    return fetch(`${this.url}/~/login`, {
      ...
      if (!this.ship && cookie) {
        const match = new RegExp(/urbauth-~([\w-]+)/).exec(cookie);
        this.ship = match ? match[1] : null;
      }
    });
  }
  ...
  async eventSource(): Promise<void> {
    ...
    if (this.lastEventId === 0) {
      useEyreState.getState().update('opening');
    ...
    return new Promise((resolve, reject) => {
      fetchEventSource(this.channelUrl, {
        ...
        onmessage: (event: EventSourceMessage) => {
          ...
          useEyreState.getState().logFact({ id: eventId, data: event.data, time: Date.now() });
          ...
          useEyreState.getState().idUpdate({ lastHeard: this.lastHeardEventId });
  ...
  private getEventId(): number {
    ...
    useEyreState.getState().idUpdate({ current: this.lastEventId });
  ...
  private async ack(eventId: number): Promise<number | void> {
    ...
    useEyreState.getState().idUpdate({ lastAcknowledged: this.lastAcknowledgedEventId });
